...
func connect(c *gin.Context) {
	...
	if correlationId != "" {
		 var correlationIdByteArray [16]byte
		//
		correlationIdSlice, _ := hex.DecodeString(correlationId)
		copy(correlationIdByteArray[:16], correlationIdSlice[:16])
		//for k, v := range []byte(correlationId) {
		//		correlationIdByteArray[k] = byte(v)
		//	}
		spanOptions := apm.SpanOptions{
			Parent: apm.TraceContext{
				Trace: apm.TraceID(correlationIdByteArray),
			},
		}
